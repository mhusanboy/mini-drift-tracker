...
async def test_edit_card_does_not_touch_the_live_screen_registry():
    # A card is a standalone record: editing it must not enroll it as a screen
    # nor clear whatever screen is currently live.
    bot = FakeBot()
    root = bot.new_message(CHAT)
    screen = await ui.show_screen(root, "menu")
    card = bot.new_message(CHAT, "card")
    await ui.edit_card(FakeCallback(bot, card), "card updated")
